...
interface SheetsLockedProps {
  className?: string;
}

export default function SheetsLocked({ className }: SheetsLockedProps) {
  return (
    <div
      className={`
      min-h-[60vh] w-full flex flex-col items-center justify-center text-center p-6 text-foreground relative
      ${className ?? ''}
    `}
    >
      {/* Ambient glows */}
      ...
      <div className="max-w-md mx-auto text-center py-16 px-6 bg-bg-subtle-alt border border-border-default rounded-2xl backdrop-blur-3xl shadow-sm relative z-10">
        <div className="w-16 h-16 bg-apple-blue/10 border border-apple-blue/20 rounded-2xl flex items-center justify-center mx-auto mb-6">
          <Link2 className="w-6 h-6 text-apple-blue" />
        </div>
        <h2 className="text-xl font-bold mb-3 text-foreground">Spreadsheet Sync Locked</h2>
        <p className="text-silver text-xs leading-relaxed mb-8">
          Connect Google Sheets to your agents to keep their knowledge base
          automatically updated from live row data.
        </p>
        <Link
          href="/billing"
