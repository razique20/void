...
function colorForType(type: Notification['type']) {
  switch (type) {
    ...
    case 'system': return 'bg-foreground/[0.04] text-silver border-foreground/[0.06]';
  }
}
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-12 w-80 max-h-[400px] bg-background/95 dark:bg-black/95 backdrop-blur-2xl border border-foreground/[0.08] dark:border-white/[0.08] rounded-2xl shadow-2xl overflow-hidden z-50 flex flex-col"
          >
            {/* Header */}
            <div className="px-4 py-3 border-b border-foreground/[0.06] dark:border-white/[0.06] flex items-center justify-between shrink-0">
              ...
              <div className="flex items-center gap-1">
                ...
                <button
                  ...
                  className="p-1 text-silver hover:text-foreground rounded-lg hover:bg-foreground/[0.04] transition-colors"
                >
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
              {notifications.length === 0 ? (
                <div className="py-12 text-center">
                  <div className="w-10 h-10 bg-foreground/[0.03] dark:bg-white/[0.03] border border-foreground/[0.06] dark:border-white/[0.06] rounded-xl flex items-center justify-center mx-auto mb-3">
                ...
                <div className="divide-y divide-foreground/[0.04] dark:divide-white/[0.04]">
                  {notifications.map((n) => (
                    <div
                      key={n.id}
                      className={cn(
                        'px-4 py-3 flex items-start gap-3 hover:bg-foreground/[0.02] dark:hover:bg-white/[0.01] transition-colors group',
            ...
            {/* Footer */}
            {notifications.length > 0 && (
              <div className="px-4 py-2.5 border-t border-foreground/[0.06] dark:border-white/[0.06] shrink-0">
                <button
                  onClick={() => {
                    markRead();
                    setOpen(false);
