          ...
          >
            ...
            <motion.div variants={itemVariants}>
              ...
              <p className="text-silver text-sm md:text-base font-medium mt-1">
                Engineer a new custom AI operative to automate your business communications.
              </p>
            </motion.div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative z-10">
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="glass border border-foreground/[0.04] dark:border-white/[0.05] p-6 rounded-[28px] space-y-4">
                    <div>
                      ...
                      <p className="text-[11px] text-silver mt-0.5">Specify a custom name for your fleet operative.</p>
                    </div>
                    ...
                    <div className="relative">
                      <input 
                        required
                        type="text"
                        placeholder="e.g. NEXUS-02, SalesBot, SupportAgent"
                        value={formData.name}
                        onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                        className="w-full bg-foreground/[0.02] dark:bg-white/[0.02] border border-foreground/5 dark:border-white/5 rounded-2xl px-5 py-4 text-sm font-bold focus:outline-none focus:border-apple-blue/40 text-foreground transition-all placeholder:text-silver/40"
                      />
                    </div>
                  </div>
                  ...
                  <div className="glass border border-foreground/[0.04] dark:border-white/[0.05] p-6 rounded-[28px] space-y-4">
                    <div>
                      ...
                      <p className="text-[11px] text-silver mt-0.5">Select the linguistic baseline parameters.</p>
                    </div>
                      ...
                      {tones.map((t) => {
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              "p-4 rounded-2xl text-left transition-all border text-foreground flex gap-3",
                              isSelected 
                                ? "bg-foreground text-background border-transparent shadow-lg shadow-foreground/5" 
                                : "bg-foreground/[0.02] dark:bg-white/[0.02] border-foreground/5 dark:border-white/5 hover:border-foreground/10 dark:hover:border-white/10 hover:bg-foreground/[0.04] dark:hover:bg-white/[0.03]"
                            )}
                          >
                            <div className={cn(
                              ...
                              <t.icon className={cn("w-4 h-4", isSelected ? "text-background" : "text-silver")} />
                            </div>
                            <div>
                  ...
                  <div className="glass border border-foreground/[0.04] dark:border-white/[0.05] p-6 rounded-[28px] space-y-4">
                    <div>
                      ...
                      <p className="text-[11px] text-silver mt-0.5">Choose the language this operative responds in.</p>
                    </div>
                        ...
                        return (
                          <button
                            ...
                            className={cn(
                              ...
                              isSelected
                                ? "bg-foreground text-background border-transparent shadow-md shadow-foreground/5"
                                : "bg-foreground/[0.02] dark:bg-white/[0.02] border-foreground/5 dark:border-white/5 hover:border-foreground/10 dark:hover:border-white/10 text-silver hover:text-foreground"
                            )}
                          >
                            {lang}
                          </button>
                  ...
                  <div className="glass border border-foreground/[0.04] dark:border-white/[0.05] p-6 rounded-[28px] space-y-4">
                    <div>
                      ...
                      <p className="text-[11px] text-silver mt-0.5">Define constraints, knowledge domains, business goals, and instructions.</p>
                    </div>
                    ...
                    <div>
                      <textarea 
                        ...
                        className="w-full bg-foreground/[0.02] dark:bg-white/[0.02] border border-foreground/5 dark:border-white/5 rounded-2xl px-5 py-4 text-xs leading-relaxed focus:outline-none focus:border-apple-blue/40 text-foreground resize-none placeholder:text-silver/40"
                      />
                    </div>
                  </div>
                  ...
                  {/* Error Notification */}
                  {error && (
                    <div className="p-4 bg-red-500/10 border border-red-500/10 rounded-2xl flex items-start gap-3">
                      <AlertTriangle className="w-5 h-5 text-red-500 flex-shrink-0 mt-0.5" />
                      <p className="text-xs text-red-500 font-semibold leading-relaxed">{error}</p>
                    </div>
                  )}
              ...
              <motion.div variants={itemVariants} className="lg:col-span-5 lg:sticky lg:top-24 space-y-4">
                ...
                <div className="glass border border-foreground/[0.04] dark:border-white/[0.05] rounded-[28px] p-6 relative overflow-hidden group shadow-2xl">
                  {/* Status Indicator */}
                  <div className="absolute top-6 right-6">
                    <div className="flex items-center gap-1.5 px-2.5 py-1 bg-emerald-500/10 border border-emerald-500/10 rounded-full">
                  ...
                  <div className="flex flex-col items-center text-center space-y-5">
                    ...
                    {/* Bot Icon */}
                    <div className="w-16 h-16 bg-foreground/[0.03] dark:bg-white/[0.02] border border-foreground/5 dark:border-white/5 rounded-2xl flex items-center justify-center shadow-inner group-hover:scale-105 transition-transform duration-300">
                    ...
                    <div>
                      ...
                      <div className="flex items-center justify-center gap-2 mt-1 text-[10px] font-bold text-silver">
                        <span className="text-apple-blue font-bold tracking-wider uppercase">{formData.tone}</span>
                        <span>•</span>
                        <span className="text-emerald-500 font-bold uppercase">{formData.language}</span>
                        <span>•</span>
                        <span className="uppercase tracking-wider">Neural Mode</span>
                      </div>
                    </div>

                    {/* Behavior Directive Display */}
                    <div className="w-full p-4 bg-foreground/[0.02] dark:bg-white/[0.02] border border-foreground/[0.03] dark:border-white/[0.03] rounded-2xl min-h-[90px] flex items-center justify-center">
