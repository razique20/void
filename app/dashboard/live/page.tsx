...
export default function LiveChatPage() {
  ...
  return (
    <div className="h-[calc(100vh-140px)] -m-8 flex overflow-hidden bg-background text-foreground transition-colors duration-300">
      {/* Conversation List */}
      <div className="w-80 flex flex-col bg-sidebar">
        <div className="p-6 border-b border-foreground/5">
          ...
          <p className="text-silver text-xs mt-1 uppercase tracking-widest font-bold">War Room</p>
        </div>
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-center mb-0.5">
                    <span className="font-bold text-sm truncate text-foreground">{chat.externalId}</span>
                    <span className="text-[10px] text-silver">{new Date(chat.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                  </div>
                  <p className="text-xs text-silver truncate">
                    {chat.messages[chat.messages.length - 1]?.content}
                  </p>
                  <div className="flex items-center gap-2 mt-2">
                    <span className="text-[10px] px-1.5 py-0.5 bg-foreground/5 rounded text-silver font-mono">
                      {chat.workerId?.name}
                    </span>
                    {chat.isPaused && (
      ...
      {selectedChat ? (
        <div className="flex-1 flex flex-col bg-background">
          {/* Chat Header */}
          <div className="p-4 border-b border-foreground/5 flex justify-between items-center bg-background/50 backdrop-blur-xl">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-xl bg-foreground/5 flex items-center justify-center">
                <User className="w-5 h-5 text-foreground" />
              </div>
              <div>
                <h2 className="font-bold text-sm text-foreground">{selectedChat.externalId}</h2>
            ...
            <div className="flex items-center gap-4">
               <div className="flex flex-col items-end mr-2">
                  <span className="text-[10px] font-bold text-silver uppercase tracking-widest">Operative</span>
              ...
              <button 
                ...
                className="p-2.5 bg-red-500/10 text-red-500 rounded-full hover:bg-red-500/20 transition-all"
                title="Clear History"
              >
          ...
          <div className="p-6 bg-foreground/5">
            <form onSubmit={handleSend} className="relative max-w-4xl mx-auto">
              <input 
                ...
                className="w-full bg-background border-none rounded-2xl pl-6 pr-16 py-4 focus:outline-none focus:ring-1 focus:ring-foreground/10 transition-all disabled:opacity-50 disabled:cursor-not-allowed text-foreground placeholder:text-silver/30"
                value={reply}
            ...
            <p className="text-center text-[10px] text-silver mt-4 uppercase tracking-[0.2em] font-medium">
